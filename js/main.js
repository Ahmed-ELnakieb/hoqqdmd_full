...
function menu_bottom_line_active() {
	var menuItem = $('#mobile-menu > ul > li.show');
	if (menuItem.length > 0) {
		var off = menuItem.offset();
		if (off) {
			var left = off.left,
				right = $(window).width() - left - menuItem.width() + menuItem.width();
...
function menu_bottom_line() {
	if ($("#mobile-menu > ul > li").length > 0) {
		$("#mobile-menu > ul > li").mouseover(function () {
			...
			var activeItem = $('#mobile-menu > ul > li.active');
			if (activeItem.length > 0) {
				var off = activeItem.offset();
				if (off) {
					var left = off.left,
						right = $(window).width() - left - activeItem.width() + activeItem.width();
